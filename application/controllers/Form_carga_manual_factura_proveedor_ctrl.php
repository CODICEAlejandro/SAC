...
class Form_carga_manual_factura_proveedor_ctrl extends CI_Controller {
	...
	public function index(){
		...
		$data['proveedores'] = $this->db->query('select id, nombre from catcliente where tipo = 1')->result();
		$data['estadosFactura'] = $this->db->query("select * from catestadofactura where id = 24")->result();
		$data["monedas"] = $this->db->query("select distinct moneda from factura")->result();

		$this->load->view("Form_carga_manual_factura_proveedor", $data);
