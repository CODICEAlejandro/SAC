...
class Reporte_master_proveedor_ctrl extends CI_Controller {
	...
	public function index(){
		...
		$data["proveedores"] = $this->db->query("SELECT * FROM catcliente WHERE estadoActivo=1 AND tipo=1 ORDER BY nombre")->result();
		$data['estadosFactura'] = $this->db->get("catestadofactura")->result();

		$this->load->view("Reporte_master_proveedor_vw", $data);
	}
	...
	public function setExcel(){
		...
		$xls->setCellValue("Proveedor"); $xls->nextCol();
